...
public class onl extends JPanel {
	...
	public enum Position {
		STRETCH, CENTER, FIT, FILL, NONE;
	}

	public onl() {
	}

	public onl(String img) {
		this(new ImageIcon(img).getImage());
	}

	public onl(Image img) {
	...
	public void paintComponent(Graphics g) {
		...
		if (this.position.equals(Position.STRETCH)) {
			...
			if (this.img != null) {
				...
				// Image img_scaled = img.getScaledInstance(scaleWidth, scaleHeight,
				// Image.SCALE_SMOOTH);
				g2.drawImage(scaleImage(img, scaleWidth, scaleHeight, getBackground()), (getWidth() - scaleWidth) / 2,
						(getHeight() - scaleHeight) / 2, scaleWidth, scaleHeight, null);
			}
		} else if (this.position.equals(Position.CENTER)) {
			if (this.img != null)
	...
	public double getScaleFactor(Dimension original, Dimension targetSize) {
		...
		if (original != null && targetSize != null) {
			...
			if (this.position.equals(Position.FIT))
				dScale = Math.min(dScaleHeight, dScaleWidth);
			else if (this.position.equals(Position.FILL))
	...
	public void setBackgroundImagePosition(String pos) {
		if ("Stretch".equals(pos))
			setBackgroundImagePosition(Position.STRETCH);
		else if ("Center".equals(pos))
			setBackgroundImagePosition(Position.CENTER);
		else if ("Fit".equals(pos))
			setBackgroundImagePosition(Position.FIT);
		else if ("Fill".equals(pos))
			setBackgroundImagePosition(Position.FILL);
		else if ("None".equals(pos))
			setBackgroundImagePosition(Position.NONE);
	}
	...
	public static void main(String[] args) {
		...
		JFrame frame = new JFrame("onl Test");
		...
		panel.setBackgroundImagePosition(onl.Position.STRETCH);
		...
		panel.setBackgroundImage("C:\\Users\\David\\Pictures\\Wood.jpg");
		panel.setBackgroundImagePosition(onl.Position.FIT);
